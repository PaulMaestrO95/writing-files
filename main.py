    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            for item in range(count):
                ...
                ingridients['ingredient_name'], ingridients['quantity'], ingridients['measure'] = ingridient.split('|')
...
def get_shop_list_by_dishes(dishes, person_count):
    ingridients_list = dict()

    for dish_name in dishes:
        if dish_name in cook_book:
            for ingridients in cook_book[dish_name]:
                meas_quan_list = dict()
                if ingridients['ingredient_name'] not in ingridients_list:
                    meas_quan_list['measure'] = ingridients['measure']
                    meas_quan_list['quantity'] = ingridients['quantity'] * person_count
                    ingridients_list[ingridients['ingredient_name']] = meas_quan_list
                else:
                    ingridients_list[ingridients['ingredient_name']]['quantity'] = \
                        ingridients_list[ingridients['ingredient_name']]['quantity'] + \
...
if __name__ == '__main__':
    ...
    print(cook_book)
    print(get_shop_list_by_dishes(dishes=['Запеченный картофель', 'Утка по-пекински'], person_count=2))
    rewrite_file()